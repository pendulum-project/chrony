...
to check a pool connecting to chrony is allowed to keep its connections alive.
The file needs to be readable by the user under which *chronyd* is running
after dropping root privileges. For security reasons, it should not be readable
by other users.
+
