...
prepare_fixedkey_request(NKSN_Instance session, int valid)
{
  ...
  if ((keytype == AEAD_AES_128_GCM_SIV) != (index == 0))
    TEST_CHECK(NKSN_AddRecord(session, 1, NKE_RECORD_FIXED_KEY, data, 2 * SIV_GetKeyLength(AEAD_AES_128_GCM_SIV)));
  else
    TEST_CHECK(NKSN_AddRecord(session, 1, NKE_RECORD_FIXED_KEY, data, 2 * SIV_GetKeyLength(AEAD_AES_SIV_CMAC_256)));
